...
function canMerge(ruleA: Rule, ruleB: Rule, browsers: string[], compatibilityCache: any) {
...
function propEquals(a: Declaration, b: Declaration, withValue = true) {
    return (a.prop === b.prop) && (!withValue || (a.value === b.value))
}
export function selectorMerger(browsers: string[], compatibilityCache: any) {
    // const debug = (msg) => {
    //     // console.log(msg);
    // }

    const caches: Rule[] = []; // contains the collection of the intersected rule cache and the origin rule (the first merged rule)
    return function (currentRule: Rule) {
        ...
        for (let cIndex = 0; cIndex < caches.length; cIndex++) {
            ...
            if (isFullyIntersect) {
                ...
                Array.from(currentRule.nodes as Declaration[]).forEach(n1 => {
                    const found = (cacheRule.nodes as Declaration[]).find(n2 => propEquals(n1, n2, false));
                    if (found) {
            ...
            const intersectedNodes = (cacheRule.nodes as Declaration[]).filter(n1 => {
                const found = (currentRule.nodes as Declaration[]).find(n2 => propEquals(n1, n2));
                if (found) {
