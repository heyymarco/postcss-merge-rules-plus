...
type PseudoElementDict = { [index: string]: string }
const pseudoElements: PseudoElementDict   = {
    ':active'           : cssSel2,
    ':after'            : cssGencontent,
...
function isCssMixin(selector: string) {
    return selector[selector.length - 1] === ':';
}

const isSupportedCache: any = {};

// Move to util in future
function isSupportedCached(feature: string, browsers: string[]): boolean {
...
export default function ensureCompatibility(selectors: string[], browsers: string[], compatibilityCache: any) {
    ...
    return selectors.every(selector => {
        ...
        postcssSelectorParser(ast => {
            ast.walk(node => {
                ...
                if (type === 'attribute' && (node as postcssSelectorParser.Attribute).attribute) {
                    // [foo]
                    if (!(node as postcssSelectorParser.Attribute).operator) {
                        compatible = isSupportedCached(cssSel2, browsers);
                    }

                    if (value) {
                        // [foo="bar"], [foo~="bar"], [foo|="bar"]
                        if (['=', '~=', '|='].includes((node as postcssSelectorParser.Attribute).operator!)) {
                            compatible = isSupportedCached(cssSel2, browsers);
                        }
                        // [foo^="bar"], [foo$="bar"], [foo*="bar"]
                        if (['^=', '$=', '*='].includes((node as postcssSelectorParser.Attribute).operator!)) {
                    ...
                    // [foo="bar" i]
                    if ((node as postcssSelectorParser.Attribute).insensitive) {
